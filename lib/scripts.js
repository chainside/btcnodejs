...
class Script {
    ...
    static compileToBuffer(str) {
        const arr = str.split(" ").reduce((arr, opcode) => {
            let res;
            try {
                res = opcodes.OpCodeConverter.toInt(opcode);
            } catch (err) {
                res = StackData.getPushOp($.hexToBytes(opcode));
            }
            return arr.concat(res);
        }, []);
        return new ByteBuffer(arr.length).append(arr).reset();
    }

    static compileToHex(str) {
        return this.compileToBuffer(str).toString('hex');
    }
}
...
class ScriptPubKey extends Script {
    ...
    static identifyFromHex(hex) {
        let buffer = ByteBuffer.fromHex(hex);
        return ScriptPubKey.identifyFromBuffer(buffer);
    }
...
class P2pkScript extends ScriptPubKey {
    constructor(source) {
        let type = "p2pk";
        let pubkey;
        let compressed;
        if (source instanceof crypto.Publickey) {
            compressed = source.type !== "uncompressed";
            pubkey = compressed ? source.compressed : source.uncompressed;
        }

        if (source instanceof ByteBuffer) {
            compressed = source.capacity() === 33;
            pubkey = source;
        }
        const keydim = compressed ? 33 : 65;
        const buffer = new ByteBuffer(keydim + 2);
        buffer
            .append($.numToBytes(keydim, 1))
            .append(pubkey.reset())
    ...
    static identify(script) {
        ...
        const compressed = ScriptPubKey.requireOp(33, buf.readUint8(0));
        const uncompressed = ScriptPubKey.requireOp(65, buf.readUint8(0));
        if (
            (compressed || uncompressed) &&
            ScriptPubKey.requireOp("OP_CHECKSIG", buf.readUint8(compressed ? 33 : 65)) &&
            script.capacity() === (compressed ? 34 : 66)
        )
            return {data: buf.copy(1, (compressed ? 33 : 65))};
...
class P2wpkhV0Script extends ScriptPubKey {
    ...
    static fromHex(hex) {
        let buffer = new ByteBuffer.fromHex(hex);
        return new P2wpkhV0Script(buffer.copy(2, buffer.capacity()));
    ...
    getScriptCode() {
        ...
        var script_len = $.numToVarInt(sc.body.capacity());
        return [script_len].concat($.hexToBytes(sc.toHex()));
...
class P2wshV0Script extends ScriptPubKey {
    constructor(source) {
        let type = "p2wsh";
        let scripthash;
        if (source instanceof ScriptPubKey) scripthash = source.p2wshHash();
        if (source instanceof address.SegwitAddress) {
            if (source.type !== type)
...
class RelativeTimelockScript extends ScriptPubKey {
    constructor(source) {
        ...
        const push_seq = StackData.opFromInt(sequence.n);
...
class TimelockScript extends ScriptPubKey {
    constructor(source) {
        let locktime, locked_script;
        if (source instanceof Array) {
            if (!(source[1] instanceof transaction.Locktime && source[0] instanceof ScriptPubKey))
                throw new TypeError("Invalid objects provided to build a TimeLockScript");
            locktime = source[1];
            locked_script = source[0];
        }
        const push_locktime = StackData.opFromInt(locktime.n);
        const buffer = new ByteBuffer(push_locktime.n + locked_script.body.capacity() + 2);

        buffer
            .append(push_locktime)
            .append($.numToBytes(Script.op_codes["OP_CHECKLOCKTIMEVERIFY"], 1))
            .append($.numToBytes(Script.op_codes["OP_DROP"], 1))
            .append(locked_script.serialize().flip());
        super(buffer);
        this.locktime = locktime;
        this.locked_script = locked_script;
        this.type = "Timelock " + this.locked_script.type;
    }
}
...
Script.identifiables = [P2pkhScript, P2pkScript, P2shScript, P2wshV0Script, P2wpkhV0Script];
module.exports = {
    ScriptSig,
    ScriptPubKey,
    P2pkhScript,
