...
class Transaction {
    ...
    serialize(segwit = this.segwit) {
        const outputSegwit = segwit && this.segwit;
        let ser_ins = [],
            ser_outs = [];
        let ser_ins_dim = 0,
            ser_outs_dim = 0;
        ...
        if (outputSegwit) dim += ser_witnesses_dim + 2;
        let buffer = new ByteBuffer(dim);

        buffer.append($.numToBytes(this.version, 4));

        if (outputSegwit) buffer.append($.numToBytes(0, 1)).append($.numToBytes(1, 1));
    ...
    toMutable() {
        var inputs = [];
        _.forEach(this.inputs, input => {
            var mutable_input = Object.assign({}, input);
            Object.setPrototypeOf(mutable_input, Input.prototype);
            inputs.push(mutable_input);
        });
    ...
    getSegwitDigestPreImage(index, prev_script, prev_amount, sighash = new Sighash("ALL")) {
        ...
        if (prev_script instanceof scripts.P2wpkhV0Script)
            ...
            var script_len = $.numToVarInt(prev_script.body.capacity());
            script_code = [script_len].concat($.hexToBytes(prev_script.toHex(0, prev_script.body.capacity())));
        }
        ...
        console.log(buffer.toHex(0, buffer.capacity()));
        console.log("Version: " + this.version);
        console.log("Hash prevouts: " + hash_prevouts.toHex(0, hash_prevouts.capacity()));
        console.log("Hash sequence: " + hash_sequence.toHex(0, hash_sequence.capacity()));
        console.log("Outpoint txid: " + curr_in.txid);
        console.log("Outpoint vout: " + curr_in.out);
        console.log("Script code: " + script_code);
        console.log("Prev amount: " + prev_amount);
        console.log("Sequence: " + curr_in.sequence.n);
        console.log("Hash outputs: " + hash_outputs.toHex(0, hash_outputs.capacity()));
        console.log("Locktime: " + this.locktime.n);
        console.log("Sighash: " + Sighash.types[sighash.sighash]);
        console.log("Digest: " + buffer.toHex(0, buffer.capacity()));
        */
        return buffer;
    }
...
class MutableTransaction extends Transaction {
    constructor(version, inputs, outputs, locktime, segwit = false) {
        super(version, inputs, outputs, locktime, segwit);
    }
    ...
    spend(txouts, solvers) {
        ...
        if (this.segwit) return this.spendSegwit(txouts, solvers);
            ...
            var spent = this.spendClassic(txouts, solvers);
            return spent;
        }
    }
...
class Sequence {
    ...
    forScript() {
        return scripts.StackData.opFromInt(this.n);
    }
}
...
const P2shSolver = require("./solvers").P2shSolver;
