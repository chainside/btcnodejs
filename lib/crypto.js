...
class Privatekey {
  ...
  static fromWIF(wif_string) {
    const ls = wif_string.substring(0, 1);
    let compressed;
    if (ls in Publickey.wif) compressed = Publickey.wif[ls] == "compressed";
    else throw new TypeError("Invalid WIF string");
    let decoded = bs58.decode(wif_string);
    let wif_key = new ByteBuffer(decoded.length).append(decoded);

    const checksum = wif_key.toHex(wif_key.capacity() - 4, wif_key.capacity());
    let prefix = wif_key.toHex(0, wif_key.capacity() - 4);
    const checksum_c = shajs("sha256")
      .update(
        shajs("sha256")
          .update(prefix, "hex")
          .digest("hex"),
      ...
      .digest("hex")
      .substring(0, 8);
    if (checksum != checksum_c) throw new AssertionError("Invalid checksum");
    if (compressed) return new Privatekey(wif_key.copy(1, wif_key.capacity() - 5));
    return new Privatekey(wif_key.copy(1, wif_key.capacity() - 4));
  }
